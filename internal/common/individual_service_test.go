...
func TestIndividualServiceConfigurationCannotFallBackToLegacyCredentials(t *testing.T) {
	for _, name := range []string{"OPENUEM_AGENT_DATABASE_URL", "OPENUEM_AGENT_BROKER_URLS", "OPENUEM_AGENT_WORKER_KEY_FILE", "OPENUEM_AGENT_BROKER_CA_FILE", "OPENUEM_AGENT_BROKER_CLIENT_CERT_FILE", "OPENUEM_AGENT_BROKER_CLIENT_KEY_FILE"} {
		t.Setenv(name, "")
	}
